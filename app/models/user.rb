class User < ApplicationRecord
  ...
  def follow(followed_user_id)
    FollowRelationship.create(following_id: followed_user_id, follower_id: id)
  end

  # ユーザーのフォローを外す
  def unfollow(followed_user_id)
    FollowRelationship.find_by(following_id: followed_user_id, follower_id: id).destroy
  end

  # フォローしていればtrueを返す
  def following?(followed_user_id)
    FollowRelationship.find_by(following_id: followed_user_id, follower_id: id).present?
  end
end
